  ...
  Promise.all([
    ...
    function renderServiceItems() {
      ...
      grid.innerHTML = categories.map(function (category) {
        var title = category['name_' + currentLang] || category.title_en;
        var description =  category['description_' + currentLang] || category.description_en;
        var src = serviceCategoryImageUrl(category);
        var width = category.image_width || '';
        var height = category.image_height || '';
        var imgAttrs = '';
        if (width) imgAttrs += ' width=\"' + escapeHtml(width) + '\"';
        if (height) imgAttrs += ' height=\"' + escapeHtml(height) + '\"';
        var imgHtml = src
          ? '<figure class=\"box-icon-image\"><img src=\"' + escapeHtml(src) + '\" alt=\"' + escapeHtml(title) + '\"' + imgAttrs + ' loading=\"lazy\" /></figure>'
          : '';
