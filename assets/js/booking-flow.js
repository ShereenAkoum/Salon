(function(){
  ...
  function renderServices(){
    ...
    state.categories.filter(c=>c.active!==false).forEach(cat=>{
      ...
      section.innerHTML=`<div class="booking-category-head"><div><h2>${esc(cat['name-'+state.lang]||cat['name-en'])}</h2></div><span class="category-count">${active.length}</span></div>`;
      const grid=document.createElement('div'); grid.className='booking-service-grid';
      active.forEach(s=>{
